...
def process_text_segmentation(text_prompts, image_path, profile):
    if not os.path.exists(lang_checkpoint):
        print(
            "⚠️ Checkpoint de LangSAM no encontrado. Por favor descargalo manualmente o implementa la función de descarga automática."
        )
    lang_sam = LangSAM()
    prompt_outputs = {}
...
def run_sam_segmentation(image_path):
    ...
    if not os.path.exists(checkpoint_path):
        raise FileNotFoundError(
            f"SAM checkpoint not found at {checkpoint_path}. Please download it."
        )
...
def calculate_class_coverage(prompt_outputs, sam_vector_out):
    ...
    coverage_html = (
        """<div style='position: fixed; bottom: 20px; left: 20px; background-color: white; border: 1px solid #aaa; padding: 10px; z-index: 1000;'>
    ...
    # Add to map HTML
    with open(
        os.path.join(output_dir, f"segmentacion_mapa_{timestamp}.html"), "a"
    ) as f:
