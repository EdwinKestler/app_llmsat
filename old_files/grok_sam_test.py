...
def create_interactive_map(bbox, zoom, vector_out, style=None):
    ...
    try:
        m = leafmap.Map(
            center=[(bbox[1] + bbox[3]) / 2, (bbox[0] + bbox[2]) / 2], zoom=zoom
        )
        m.add_basemap("Esri.WorldImagery")
        m.add_vector(
            vector_out, layer_name="Segmentación sin árboles/agua", style=style
        )
...
def main():
    ...
    with rasterio.open(image_path) as src:
        profile = src.profile
    combined_mask, multi_mask_path = combine_text_masks(
        text_prompts, image_path, profile
    )
