    ...
    for cloud_limit in cloud_limits:
        try:
            ...
            if not data.get("features"):
                logger.warning(
                    f"No Sentinel-2 data found with cloud cover <= {cloud_limit}%."
                )
                continue
            latest_date = data["features"][0]["properties"]["startDate"].split("T")[0]
            logger.info(
                f"Latest Sentinel-2 date found with cloud cover <= {cloud_limit}%: {latest_date}"
            )
            return latest_date
        except requests.HTTPError as e:
            logger.error(
            ...
            logger.error(
                f"Error querying CDSE Catalog API with cloud cover {cloud_limit}%: {e}"
            )
...
def calculate_segmentation_urbanization(
    building_vector: str, road_vector: str, total_area: float
) -> float:
...
def calculate_vegetation_precipitation(
    tree_vector: str, precip_path: str, total_area: float
) -> tuple:
...
def main():
    ...
    gee_layers = [
        (
            ...
            if is_valid_file(base_image_path)
            else None
        ),
        (
