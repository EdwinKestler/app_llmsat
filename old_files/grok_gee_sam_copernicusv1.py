...
def get_cdse_access_token(
    ...
    try:
        ...
        logger.error(
            f"HTTP error obtaining CDSE access token: {e.response.status_code} {e.response.text}"
        )
        return None
...
def get_latest_sentinel2_date(
    bbox: list, access_token: str, max_cloud_cover: int = 50
) -> Optional[str]:
    ...
    for cloud_filter in cloud_filters:
        try:
            ...
            logger.error(
                f"HTTP error querying CDSE Catalog API: {e.response.status_code} {e.response.text}"
            )
            if e.response.status_code == 400:
                logger.warning(
                    "CloudCover field likely invalid—retrying without cloud filter."
                )
                continue
            if e.response.status_code == 403:
                logger.warning(
                    "403 Forbidden: Clearing cached token and retrying authentication."
                )
                token_file = os.path.join(CONFIG["output_dir"], "cdse_token.json")
                if os.path.exists(token_file):
...
def get_latest_sentinel2_date_fallback(
    bbox: list, max_cloud_cover: int = 50
) -> Optional[str]:
    ...
    try:
        ...
        if latest_image is None:
            logger.warning(
                "No Sentinel-2 data found in GEE for the given bbox and time range."
            )
            return None
        latest_date = (
...
def validate_bbox(bbox: list) -> list:
    ...
    if not isinstance(bbox, (list, tuple)) or len(bbox) != 4:
        raise ValueError(
            "bbox must be a list/tuple of 4 values: [minx, miny, maxx, maxy]"
        )
    minx, miny, maxx, maxy = bbox
    if not (-180 <= minx <= maxx <= 180 and -90 <= miny <= maxy <= 90):
        raise ValueError(
            "bbox coordinates out of range: longitudes [-180, 180], latitudes [-90, 90]"
        )
    ...
    if area < 0.0001:
        logger.warning(
            f"Small bbox area ({area:.6f} sq. degrees) may cause empty exports."
        )
    return bbox
...
def export_gee_image(
    image: ee.Image, region: ee.Geometry, filename: str, scale: int = 30
) -> str:
...
def export_sentinel2(bbox: list, timestamp: str, config: dict) -> str:
    ...
    try:
        ...
        if not (username and password):
            logger.warning(
                "CDSE credentials not found in environment variables. Prompting for input."
            )
        ...
        if not (username and password):
            ...
            if not token_data and totp is None:
                logger.warning(
                    "First CDSE authentication attempt failed. Retrying with 2FA."
                )
                totp = getpass("Enter 2FA code: ") or None
                token_data = get_cdse_access_token(username, password, totp=totp)
            if not token_data:
                logger.warning(
                    "Failed to obtain CDSE access token. Trying GEE fallback."
                )
                latest_date = get_latest_sentinel2_date_fallback(
                    bbox, config["gee_export"]["s2_cloud_percentage"]
                )
            else:
                latest_date = get_latest_sentinel2_date(
                    bbox,
                ...
                if not latest_date:
                    logger.warning(
                        "CDSE Catalog query failed. Retrying with new token."
                    )
                    token_file = os.path.join(CONFIG["output_dir"], "cdse_token.json")
                    if os.path.exists(token_file):
                        os.remove(token_file)
            ...
            if not latest_date:
                logger.warning(
                    "No valid Sentinel-2 date found. Using fallback date range."
                )
                start_date = config["gee_export"]["s2_date_range"][0]
                end_date = config["gee_export"]["s2_date_range"][1]
            else:
                ...
                start_date = (
                    datetime.strptime(latest_date, "%Y-%m-%d") - timedelta(days=5)
                ).strftime("%Y-%m-%d")
        ...
        if s2 is None:
            logger.warning(
                "No valid Sentinel-2 image found for the given bbox and date range."
            )
            return None
        rgb = s2.select(["B4", "B3", "B2"])
        return export_gee_image(
            rgb,
            region,
            os.path.join(config["data_dir"], f"s2_rgb_{timestamp}.tif"),
            scale=10,
        )
    except ee.EEException as e:
...
def export_dem_and_slope(bbox: list, timestamp: str, config: dict) -> tuple:
    ...
    try:
        ...
        dem_path = export_gee_image(
            dem,
            region,
            os.path.join(config["data_dir"], f"dem_glo30_{timestamp}.tif"),
            scale=config["gee_export"]["dem_scale"],
        )
        slope_path = export_gee_image(
            slope,
            region,
            os.path.join(config["data_dir"], f"slope_glo30_{timestamp}.tif"),
            scale=config["gee_export"]["dem_scale"],
        )
...
def export_cgls_lc100(bbox: list, timestamp: str, config: dict) -> str:
    ...
    region = ee.Geometry.Rectangle(bbox)
    try:
        landcover = ee.Image("COPERNICUS/Landcover/100m/Proba-V-C3/Global/2019").select(
            "discrete_classification"
        )
        return export_gee_image(
            landcover,
            region,
            os.path.join(config["data_dir"], f"cgls_lc100_{timestamp}.tif"),
...
def export_era5_precipitation(bbox: list, timestamp: str, config: dict) -> str:
    ...
    try:
        ...
        return export_gee_image(
            era5,
            region,
            os.path.join(config["data_dir"], f"era5_precipitation_{timestamp}.tif"),
...
def export_gee_layers(bbox: list, timestamp: str, config: dict) -> tuple:
    ...
    with ThreadPoolExecutor(max_workers=3) as executor:
        future_to_task = {
            executor.submit(func, bbox, timestamp, config): name for func, name in tasks
        }
        ...
        with rasterio.open(input_path) as src:
            ...
            for b in range(arr.shape[0]):
                ...
                scaled = (
                    np.zeros_like(band)
                    if maxval <= minval
                    else ((band - minval) / (maxval - minval)) * 255
                )
    ...
    try:
        with rasterio.open(image_path) as img_src, rasterio.open(
            ...
            if slope.shape != image.shape[1:]:
                slope = ndimage.zoom(
                    slope,
                    (image.shape[1] / slope.shape[0], image.shape[2] / slope.shape[1]),
                    order=1,
                )
...
def get_valid_base_image(
    bbox: list, timestamp: str, config: dict
...
# === Segmentation ===
def process_text_segmentation(
    text_prompts: list, image_path: str, config: dict
) -> dict:
        ...
        for prompt in tqdm(text_prompts, desc="Segmenting prompts"):
            try:
                lang_sam.predict(
                    ...
                    text_threshold=config["prompt_thresholds"][prompt][
                        "text_threshold"
                    ],
                )
                ...
                for geom, value in shapes(mask, transform=transform):
                    ...
                    props.append(
                        {"type": prompt, "label": f"{prompt.capitalize()} Feature"}
                    )
                if not geoms:
                    logger.warning(f"No valid polygons for {prompt}")
                    continue
                vector_out = os.path.join(
                    config["output_dir"], f"segment_{prompt}_{timestamp}.gpkg"
                )
...
# === Metrics and Mapping ===
def calculate_segmentation_urbanization(
    building_vector: str, bridge_vector: str, total_area: float
) -> float:
...
def calculate_vegetation_precipitation(
    tree_vector: str, river_vector: str, precip_path: str, total_area: float
) -> tuple:
    ...
    if is_valid_file(precip_path):
        try:
            with rasterio.open(precip_path) as src:
                ...
                avg_precip = (
                    round(precip.mean() * 1000, 2) if precip.size > 0 else 0
                )  # Convert m to mm
        except Exception as e:
            logger.error(f"Error reading {precip_path} for precipitation: {e}")
    else:
        logger.warning(
            "Precipitation data unavailable; setting average precipitation to 0 mm."
        )
...
def add_segmentation_layers(
    m: folium.Map, prompt_outputs: dict, sam_vector_out: str, layer_styles: dict
) -> None:
...
def add_metrics_panel(
    m: folium.Map, prompt_outputs: dict, precip_path: str, total_area: float
) -> None:
...
def create_interactive_map(
    bbox: list, zoom: int, prompt_outputs: dict, sam_vector_out: str, gee_layers: list
) -> None:
    ...
    precip_path = next(
        (l["path"] for l in gee_layers if l["name"].startswith("ERA5")), None
    )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--zoom", type=int, default=CONFIG["zoom"], help="Zoom level for TMS"
    )
    parser.add_argument(
        "--output-dir", default=CONFIG["output_dir"], help="Output directory"
    )
...
def main():
    ...
    dem_path, slope_path, cgls_path, precip_path = export_gee_layers(
        config["bbox"], timestamp, config
    )
    ...
    prompt_outputs = process_text_segmentation(
        text_prompts, enhanced_image_path, config
    )
    ...
    gee_layers = [
        ...
        (
            {"name": "DEM Copernicus", "path": dem_path, "bands": [1], "opacity": 0.5}
            if is_valid_file(dem_path)
            else None
        ),
        (
            {
                "name": "Slope Copernicus",
                "path": slope_path,
                "bands": [1],
                "opacity": 0.5,
            }
            if is_valid_file(slope_path)
            else None
        ),
        (
            {"name": "CGLS-LC100", "path": cgls_path, "bands": [1], "opacity": 0.4}
            if is_valid_file(cgls_path)
            else None
        ),
        (
            {
    ...
    create_interactive_map(
        config["bbox"], config["zoom"], prompt_outputs, sam_vector_out, gee_layers
    )
